...
def decisionTree(train, test):
    ...
    X_train,  y_train, = train_test_split(train, test_size=0.2, random_state=42)
    #X_train, X_test, y_train, y_test = train_test_split(train, test, test_size=0.2, random_state=42)
    #pca = PCA(n_components=2)  # Defina o número de componentes principais desejados
    #X_train_pca = pca.fit_transform(X_train)
    #X_test_pca = pca.transform(X_test)
    DT = DecisionTreeClassifier()
    dataframeGPA = X_train[" gpa 13s"]
    del X_train[" gpa 13s"]
    #print(y_train)
    DT.fit(X_train, dataframeGPA)

    dataframeGPA = y_train[" gpa 13s"]
    del y_train[" gpa 13s"]
    ...
    # Calcular a acurácia
    accuracy = accuracy_score(dataframeGPA, y_pred)
